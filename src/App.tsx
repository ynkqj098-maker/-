...
const App: React.FC = () => {
  ...
  const deleteHealthLog = useCallback((idToDelete: string) => {
    setHealthLogs(prevLogs => prevLogs.filter(log => log.id !== idToDelete));
  }, []);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-200">
      ...
      <main className="container mx-auto max-w-2xl p-4 md:p-6">
        <div className="space-y-8">
          ...
          <History logs={healthLogs} onDeleteLog={deleteHealthLog} />
        </div>
      </main>
    </div>
  );
